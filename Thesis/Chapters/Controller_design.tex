...
The finger model from \cref{sec:finger_model} is used to simulate the dynamics of the finger in response to the force, and to calculate the force it puts on the cables. The parameters for the motor and finger model are given in [\textbf{TODO: Add table of model parameters}]. Both cables are initialized to be under tension when the finger is in a straight position, i.e. when $\varphi_{\jnt{MCP}} = \varphi_{\jnt{PIP}} = \varphi_{\jnt{DIP}} = 0$. To make sure the motors aren't working against each other, they receive an opposite reference angle when the controller is active. This ensures that as one motor pulls on the finger, increasing the tension in its cable, the other motor will be pulling in the opposite direction, decreasing the tension in its cable. 
