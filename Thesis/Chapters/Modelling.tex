...
To facilitate the controller design and simulations in \cref{ch:controller_design}, this chapter presents the dynamic models to describe the exoskeleton. First, a finger model is derived using the Euler--Lagrange equations. The brushed DC motor dynamics is then introduced based on \textcite{rahmanMathematicalModelBrushed2021}, and relevant motor parameters are estimated from the manufacturer datasheet. Finally, simplified load models for the motor and cable constraints are introduced to describe the interaction between the motors and the finger.


\section{Finger model}%
\label{sec:finger_model}

For modelling the finger, we assume that the motion is limited to a 2D-plane. This is mostly true, due to the collateral ligaments at the DIP- and PIP joints, preventing excessive side-to-side bending, as discussed in \cref{subsec:theory_hand_anatomy}. The MCP joint has a different support-structure, that makes it free to move laterally when the finger is extended. But this isn't as relevant to model for the task at hand, as the lateral movement from the cable pulling on the finger should be minimal.

For the model, the phalanges will be modelled as three links, with three revolute joints, like a \textit{planar manipulator} \parencite{PlanarManipulatorOverview}. To model the stiffness from the tendons and ligaments, we will add a torsional spring at each joint. The springs will have a non-zero equilibrium angle, representing the natural resting position of the finger joints. The damping from the soft tissue will be modelled by torsional dampers at each joint. A schematic of the model can be seen in \cref{fig:modelling_finger_model}. We will model the links as rigid cylinders, with uniform mass distribution, and the joints as ideal revolute joints, with no friction. The parameters of the model are listed in \cref{tab:finger_params}. It should be noted, that the parameters of the finger model are not taken from any literature, but were selected empirically through repeated simulations of an unloaded finger to obtain realistic motion. 
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        \multirow{3}{*}{Mass}
            & $m_{\jnt{PP}}$ & $20$ & \multirow{3}{*}{$\si{\gram}$} \\
            & $m_{\jnt{MP}}$ & $15$ & \\
            & $m_{\jnt{DP}}$ & $20$ & \\
        \midrule
        \multirow{3}{*}{Length}
            & $l_{\jnt{PP}}$ & $48$ & \multirow{3}{*}{$\si{\milli\metre}$} \\
            & $l_{\jnt{MP}}$ & $30$ & \\
            & $l_{\jnt{DP}}$ & $24$ & \\
        \midrule
        \multirow{3}{*}{Radius}
            & $r_{\jnt{PP}}$ & $8.5$ & \multirow{3}{*}{$\si{\milli\metre}$} \\
            & $r_{\jnt{MP}}$ & $8.5$ & \\
            & $r_{\jnt{DP}}$ & $8.5$ & \\
        ...
        \multirow{3}{*}{Equilibrium angle}
            & $\varphi_{\jnt{MCP},\mathrm{eq}}$ & $\pi/6$ & \multirow{3}{*}{$\si{\radian}$} \\
            & $\varphi_{\jnt{PIP},\mathrm{eq}}$ & $\pi/4$ & \\
            & $\varphi_{\jnt{DIP},\mathrm{eq}}$ & $\pi/12$ & \\
        \midrule
        \multirow{3}{*}{Stiffness coefficient}
            & $k_{\jnt{MCP}}$ & $0.5$ & \multirow{3}{*}{$\si{\newton\metre\per\radian}$} \\
            & $k_{\jnt{PIP}}$ & $0.5$ & \\
            & $k_{\jnt{DIP}}$ & $0.5$ & \\
        \midrule
        \multirow{3}{*}{Damping coefficient}
            & $b_{\jnt{MCP}}$ & $0.05$ & \multirow{3}{*}{$\si{\newton\metre\second\per\radian}$} \\
            & $b_{\jnt{PIP}}$ & $0.05$ & \\
            & $b_{\jnt{DIP}}$ & $0.05$ & \\
        \bottomrule
    \end{tabular}
...
To model the dynamics of the finger, we will use the Euler--Lagrange equations as shown in \cref{subsec:theory_modelling_euler_lagrange_equations}. We will define our generalized coordinates as the relative joint angles, $\varphi_{\jnt{MCP}}$, $\varphi_{\jnt{PIP}}$ and $\varphi_{\jnt{DIP}}$:
...
The translational kinetic energy is a bit more complex to derive, as the linear velocity of each link depends on the geometry of the system. The linear velocity of each link can be derived by finding the center of mass of each link and taking the time derivative. To find the center of mass, we will use homogeneous transformations, as described in \cref{subsec:theory_modelling_homogeneous_transformations}. 

Let's start by defining frames for moving between the links. We will define the static world frame as frame $i$, attached to the MCP joint, where the x-axis is aligned with the metacarpal. The next is frame $PP$, attached to the PIP joint, where the x-axis is aligned with the proximal phalanx. Subsequently we have frame $MP$, attached to the DIP joint, where the x-axis is aligned with the middle phalanx. Finally, we have frame $DP$, attached to the tip of the finger, where the x-axis is aligned with the distal phalanx. A visualization of the frames is shown in \cref{fig:modelling_finger_model_frames}. The homogeneous transformations between these frames can be expressed as:

\begin{equation}
    \label{eq:modelling_homogeneous_transformations}
    \begin{aligned}
...
\begin{figure}
    \centering
    \includegraphics[width=1.0\textwidth]{../Figures/Modelling/finger_variables_visualization_test.pdf}
    \caption{Visualization of the finger model, with three revolute joints, where the relative joint angles, and the lengths of each phalanx are labeled, with frames.}
    \label{fig:modelling_finger_model_frames}
\end{figure}
...
Extracting the x and y components of the position, defining them as $\mathbf{r}_{\jnt{PP, CM}}$, $\mathbf{r}_{\jnt{MP, CM}}$, and $\mathbf{r}_{\jnt{DP, CM}}$, we can take the time derivative to find the linear velocity of each center of mass, which can then be used to calculate the total translational kinetic energy as:
...
For the potential energy, one would normally consider the gravitational potential energy. However, since the finger is relatively light and the motion during gripping tasks is mostly transverse to gravity, we will omit the gravitational potential energy for simplicity. For clarity, we will model the torsional springs as generalized forces, instead of including them in the potential energy. Hence, the potential energy of the system can be expressed as $V = 0$.

\subsection{Generalized forces}

The generalized forces in our system will be the torques from the torsional springs, $\boldsymbol{\tau_{\text{spring}}}$, and dampers, $\boldsymbol{\tau_{\text{damper}}}$, as well as the torques from the cable pulling on the finger, $\boldsymbol{\tau_{\text{cable}}}$, and other external torques when the finger interacts with objects, $\boldsymbol{\tau_{\text{ext}}}$. The torques from the torsional springs can be expressed as:
...
\begin{align*}
    ...
    -p_3 \dot{\varphi}_{23} s_{\varphi_{23}}
    ...
    + p_4 \dot{\varphi}_{12} s_{\varphi_3},
    \\
    \gamma_{32}
    &= p_4 \dot{\varphi}_{12} s_{\varphi_3}.
\end{align*}

 The generalized forces $\boldsymbol{\tau}$ are the same as in \cref{eq:modelling_total_generalized_forces}. The \textit{gravity vector} $\mathbf{G}(\mathbf{q})$ represents the gravitational forces acting on the system. Since we're omitting the effects of gravity, the gravity vector is zero:
...
In the physical system, $\SI{12}{\volt}$ brushed DC motors from Pololu \parencite{Pololu751} are used. The dynamical model of the brushed DC motor is based on \cite{rahmanMathematicalModelBrushed2021}. The circuit diagram of the brushed DC motor is shown in \cref{fig:motor_circuit}. \cref{eq:motor_model_mechanical} describes the mechanical dynamics of the motor, and \cref{eq:motor_model_electrical} describes the electrical dynamics of the motor, defined as:
...
Here, $\theta$ defines the output angle of the motor. The torque generated by the motor is given by $\tau_m = K_t I_a$, where $K_t$ is the torque constant of the motor. The load torque $\tau_l$ represents any external torque applied to the motor. The friction within the rotor of the motor is modelled with $B_m \frac{\mathrm{d} \theta}{\mathrm{d}t}$, where $B_m$ is a constant friction coefficient. The output inertia of the rotor is given by $J_m$. The back electromotive force (EMF) is given by $E_b = K_b \frac{\mathrm{d} \theta}{\mathrm{d}t}$, where $K_b$ is the back EMF constant of the motor. $E_a$ is the armature voltage, which serves as the control input to the system. $R_a$ and $L_a$ are the armature resistance and inductance, respectively. With the change to $\tau_m$ and $E_b$, we can rewrite the equations as:
...
It's not possible to estimate the inertia of the rotor $J_m$, nor the armature inductance $L_a$ from the datasheet. For simulation purposes, the parameters are chosen as $J_m=\SI{0.093}{\kilogram \metre\squared}$ and $L_a=\SI{6}{\milli\henry}$, reported as example brushed DC motor values by \textcite{rahmanMathematicalModelBrushed2021}. These values can be estimated through a calibration step later. One possible approach is to estimate the parameters using a parameter identification method.

\subsection{Estimating the armature inductance and rotor inertia}%
\label{subsec:est_armature_inductance}

To estimate the armature inductance and rotor inertia, we can use a parameter identification method. Converting \cref{eq:motor_model_electrical_rewrite} to the \textit{Laplace domain} \parencite{LaplaceTransformOverview}, gives:
...
Rearranging, moving all measured signals and known parameters to the left, and all factors with the unknown parameter $L_a$ to the right, we get:
...
Hence we've transformed the system to a SPM like discussed in \cref{subsec:theory_control_static_parametric_model}. We can do the same for the inertia of the rotor $J_m$. We will start by converting \cref{eq:motor_model_mechanical_rewrite} to the Laplace domain, assuming there is no external load ($\tau_l \equiv 0$), giving us:
...
Applying the filter to \cref{eq:motor_model_mechanical_laplace}, moving all measured signals and known parameters to the left, and all factors with the unknown parameter $J_m$ to the right, we get:
...
\begin{equation}
    ...
    \begin{bmatrix}
        L_a \\
        J_m
    \end{bmatrix}}_{\textstyle{\boldsymbol{\theta}^*}};
\end{equation}

\begin{equation*}
    \Longrightarrow \quad \mathbf{z} = \boldsymbol{\Phi} \boldsymbol{\theta}^*.
\end{equation*}
...
\begin{figure}%
    \centering
    \includegraphics[width=0.7\textwidth]{../Figures/Modelling/inertia_and_inductance_estimator_gradient_results.pdf}
...
In \cref{sec:finger_model} we assumed that the finger could be modelled as several torsional mass-spring-damper systems. Since we can't measure the finger-angles, and that the configuration of the whiffletree is unknown, the full finger model isn't observable, and can't be used for position control design. For analysis, we will instead assume that the finger can be modelled as a single mass-spring-damper system, acting on the motor as $\tau_l$. This is a simplification, but it allows for a better analysis of the system. Hence we will model the load torque $\tau_l$ as:

\begin{equation}
    \tau_l = J \frac{\mathrm{d}^2\theta}{\mathrm{d}t^2} + c \frac{\mathrm{d}\theta}{\mathrm{d}t} + k (\theta - \theta_{eq}),
\end{equation}

where $J$ is the 'total' inertia of the finger, $c$ is the 'total' damping coefficient of the finger, $k$ is the 'total' stiffness of the finger, and $\theta_{eq}$ is the 'total' equilibrium angle of the finger. Substituting this into \cref{eq:motor_model_mechanical_rewrite}, we get the updated mechanical dynamics for the brushed DC motor with the simplified finger load:
...
\subsection{State-space model of brushed DC motor with simplified finger load}%
\label{subsec:state_space_model_load}

For the position controller for the motor, discussed further in \cref{sec:mrac_implementation}, we need to express the dynamics in \cref{eq:motor_model_mechanical_load} in state-space representation, as shown in \cref{subsec:theory_control_state_space}. Assuming that $I_a$ can be treated as an input, explained in \cref{sec:controller_design_current_controller}, and defining $\omega\triangleq\frac{\mathrm{d}\theta}{\mathrm{d}t}$, we can rewrite \cref{eq:motor_model_mechanical_load} and as:
...
In \cref{sec:mrac_implementation}, we implement a position controller for the brushed DC motor. For evaluation, we will simulate two motors, one for flexion and one for extension, as is the case in our physical system. The motors will be pulling on their own cables, where the force which the motors generate gets distributed to the three phalanges through their own \textit{whiffletree}. As the exact configuration is still being worked on, we will simply define that $w_1 = \frac{1}{2}$ of the force pulling on the whiffletree will go to the proximal phalanx, $w_2 = \frac{1}{3}$ of the force going to the middle phalanx, and $w_3 = \frac{1}{6}$ of the force going to the distal phalanx. These values are subject to change in the actual system. The force application is shown in \cref{fig:controller_design_force_application}. 
...
where $r_{spindle}$ is the radius of the spindle mounted on the motor, $\theta$ is the angle of the motor, $L_{finger}(\mathbf{q})$ is an estimate for the total length of cable from the finger to the whiffletree. $C$ is a constant defining the total cable length in the system, which then constrains the angle of the motor and the relative angles of the finger. The estimate of the total cable going from the finger to the whiffletree is defined as:

\begin{equation}
    L_{finger}(\mathbf{q}) = \sum_{i = 1}^{3} w_i ||\mathbf{r}_{a, i} - \mathbf{r}_{t, i}||,
\end{equation}

where $\mathbf{r}_{a, i}$ and $\mathbf{r}_{t, i}$ are the attachment and target points of the $i$-th phalanx, respectively, shown in \cref{fig:controller_design_force_application}. The distance between the attachment and target points of the $i$'th phalanx is scaled with the force ratio $w_i$ to get a better estimate of the total length of cable with the whiffletree. Differentiating the constraint, we get the velocity constraint:
...
To reduce numerical drift, Baumgarte stabilization is added \parencite{baumgarteStabilizationConstraintsIntegrals1972}, to combine all the constraints into a single equation:
...
Since the cables can only pull on the finger, we further impose that the cable tensions must be non-negative:

\begin{equation}
    T_f, T_e \geq 0.
\end{equation}

To be clear, this implementation of the cable tension is not a perfect emulation of the real system. In reality, the cable is slightly elastic, and the whiffletrees will provide some friction. There will also be more cable in the system. However, considering that the position of the attachment points, target points, and configuration of whiffletrees are not yet decided in the real system, this should provide a good enough approximation to test the position controller, and get a better understanding of the dynamics of the system.

\subsubsection{Updated dynamics with the cable tension}

Given the cable tension $T_x$ prior to the whiffletree for the $x$-th cable, the $x$-th brushed DC motor experiences a load torque of $\tau_l = r_{spindle} T_x$, giving the updated mechanical dynamics from \cref{eq:motor_model_mechanical_rewrite}:
...
\begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth]{../Figures/Controller_design/force_diagram.pdf}
    \caption{Force application on the finger. Shows how the force from the flexion and extension motors gets distributed after the whiffletrees.}
