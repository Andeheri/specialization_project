...
\begin{figure}[htbp]%
    ...
    \label{fig:modelling_parameters}
\end{figure}

To model the dynamics of the finger, we'll use the Euler-Lagrange equations as shown in (\textbf{Add reference to theory: Euler-Lagrange Equations}). We'll define our generalized coordinates as the relative joint angles, $\varphi_{\jnt{MCP}}$, $\varphi_{\jnt{PIP}}$ and $\varphi_{\jnt{DIP}}$:
...
\begin{equation}
    ...
    T_{\text{trans}} = \frac{1}{2} \left [m_{\jnt{PP}}\dot{\mathbf{r}}_{\jnt{PP, CM}}^T\dot{\mathbf{r}}_{\jnt{PP, CM}} + m_{\jnt{MP}}\dot{\mathbf{r}}_{\jnt{MP, CM}}^T\dot{\mathbf{r}}_{\jnt{MP, CM}} + m_{\jnt{DP}}\dot{\mathbf{r}}_{\jnt{DP, CM}}^T\dot{\mathbf{r}}_{\jnt{DP, CM}}\right ].
\end{equation}
...
Sadly it's not possible to estimate the inertia of the rotor $J_m$, nor the armature inductance $L_a$ from the datasheet. For simulation purposes, we will define $J_m=\SI{0.093}{\kilo \gram \meter^2}$ and $L_a=\SI{6}{\milli\henry}$, which were given as common values brushed DC motors in [\textbf{Cite to article describing Brushed DC Motor}]. Even though we don't know the values now, they can be estimated on the actual motor through a calibration step later. There are many ways to do this. One way that's probably a bit overkill, but that I think will be the most convenient, is to use a parameter identification method. 
...
\textbf{TODO: Add the same for the inertia of the rotor.}
