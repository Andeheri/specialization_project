...
In \cref{sec:finger_model} we assumed that the finger could be modelled as several torsional mass-spring-damper systems. Since we can't measure the finger-angles, and that the configuration of the whiffle tree is unknown, the full finger model isn't observable, and can't be used for position control design. For analysis, we will instead assume that the finger can be modelled as a single mass-spring-damper system, acting on the motor as $\tau_l$. This is a simplification, but it allows for a better analysis of the system. Hence we will model the load torque $\tau_l$ as:
...
\begin{figure}
    ...
    \caption{Force application on the finger. Shows how the force from the flexion and extension motors gets distributed after the whiffle trees.}
