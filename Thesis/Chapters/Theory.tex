...
A way to express a nth order ordinary differential equation (ODE) is as a system of n first order ODEs. This is called the \textit{state-space representation} of the system. A state-space representation commonly used in control theory for \textit{linear time-invariant} (LTI) systems is the following:
\begin{equation}
...
\textcite{ioannouRobustAdaptiveControl2012} proves, that $\mathbf{K}(t)$, $\mathbf{L}(t)$, and $\boldsymbol{\epsilon}(t)$ are bounded, and the tracking error $\boldsymbol{\epsilon}(t) \to 0$ as $t \to \infty$.