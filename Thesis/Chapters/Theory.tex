...
To facilitate the modelling of the planar finger model in \cref{ch:modelling}, the following modelling theory is presented. First, the \textit{Euler--Lagrange equations}, as a way to derive the equations of motion for the finger, and \textit{homogenous transformations}, used to move between frames when deriving the kinematics of the finger. Both are well known concepts in the field of robotics, but the following definitions were based on \cite{freidovichControlMethodsRobotic2025}.
...
\begin{equation}
    \frac{\mathrm{d}}{\mathrm{d}t} \left( \frac{\partial \mathcal{L}}{\partial \dot{\mathbf{q}}} \right) - \frac{\partial \mathcal{L}}{\partial \mathbf{q}} = \mathbf{Q}, b
\end{equation}
...
\cite{ioannouRobustAdaptiveControl2012} proves, that $\mathbf{K}(t)$, $\mathbf{L}(t)$, and $\boldsymbol{\epsilon}(t)$ are bounded, and the tracking error $\boldsymbol{\epsilon}(t) \to 0$ as $t \to \infty$.