...
When realizing a controller on a digital computer, the system is often modelled as a discrete-time system, as the samples are taken at discrete time steps. A sample of state $y$ at time step $k$ is denoted as $y[k]$. The sampling time, denoted by $T_s$, is the time between two consecutive samples, such that if $y[k] = y(t)$, then $y[k+1] = y(t + T_s)$. Usually the input of the system is denoted $u[k]$. Often the discrete-system can be expressed as a \textit{auto-regressive} (AR) model, where the current state of the system is given by a linear combination of the previous states and inputs, as shown in the following equation:
...
\end{equation}
where $h(z)$ denotes the system transfer function. For a causal discrete-time
...
A \textit{static parametric model} (SPM) is a special form of a dynamical system relating the input and output of a system when there are unknown parameters. \cite[p. 13]{ioannouAdaptiveControlTutorial2006} defines a SPM as follows:
...
\subsection{Gradient method for estimating unkown parameters}
...
\begin{equation}
    \int_{t}^{t+T} \boldsymbol{\phi}(\tau) \boldsymbol{\phi}^\intercal(\tau) \mathrm{d}\tau \geq \alpha \mathbf{I}, \quad \forall t \geq 0.
\end{equation}

Essentially, this means that $\boldsymbol{\phi}$ must be sufficiently rich in frequency content, and cannot be a constant signal.
...
\begin{equation}
    \begin{aligned}
        \dot{\mathbf{K}}(t) &= \mathbf{B}_m^\intercal \mathbf{P} \mathbf{e} \mathbf{x}^\intercal sgn(l), \\
        \dot{\mathbf{L}}(t) &= -\mathbf{B}_m^\intercal \mathbf{P} \mathbf{e} \mathbf{r}^\intercal sgn(l).
    \end{aligned}
\end{equation}

\cite{ioannouRobustAdaptiveControl2012} proves, that $\mathbf{K}(t)$, $\mathbf{L}(t)$, and $\mathbf{e}(t)$ are bounded, and the tracking error $\mathbf{e}(t) \to 0$ as $t \to \infty$.