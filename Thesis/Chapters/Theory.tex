...
If $\text{rank}(\mathcal{C}) = n$, where $n$ is the dimension of the state vector $\mathbf{x}$, the system $(\mathbf{A}, \mathbf{B})$ is said to be \textit{controllable}.

\subsection{Model Reference Adaptive Control (MRAC)}%
\label{subsec:theory_control_mrac}

\textit{Model Reference Adaptive Control} (MRAC) is a control strategy that aims to make the output of a system follow the output of a reference model, even in the presence of parameter uncertainties. The following definition of MRAC is based on \textcite[325--327]{ioannouRobustAdaptiveControl2012}. 
...
Hence the closed-loop dynamics of $\mathbf{x}(t)$ is the same as \cref{eq:theory_control_reference_model}, and $\mathbf{x}(t) \to \mathbf{x}_m(t)$ exponentially fast. But for most cases, $\mathbf{A}$ and $\mathbf{B}$ are unknown, which means that $\mathbf{K^*}$ and $\mathbf{L^*}$ has to be estimated online. Assuming that $\mathbf{K^*}$ and $\mathbf{L^*}$ exist, we'll change the input in \cref{eq:theory_control_ideal_mrac_input} to:
